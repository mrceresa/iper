...
class CityModel(MultiEnvironmentWorld):
    ...
    def step(self):
        ...
        if current_step.day != self.DateTime.day:
            ...
            S, E, I, R, H, D = _t["Susceptible"][-1], _t["Exposed"][-1], _t["Infected"][-1], _t["Recovered"][-1], \
                               _t["Hospitalized"][-1], _t["Dead"][-1]
    ...
    def createAgents(self, Humanagents, Workplaces, friendsXagent=3):  # friendsXagent=3
        ...
        while agentsToBecreated >= 0:
            if isinstance(self._agentsToAdd[agentsToBecreated], HumanAgent):
                ...
                self._agentsToAdd[agentsToBecreated].pos = (self.Ped_Map.G.nodes[node]['x'],
                ...
                for i in range(0, family_dist[index]):
                    ...
                    print("++++++++++++++++++++", "id agente", self._agentsToAdd[agentsToBecreated - i].id,
                          "   posizione iniziale agente", self._agentsToAdd[agentsToBecreated - i].pos)
                    for y in range(0, family_dist[index]):
                        if i != y:
                            self._agentsToAdd[agentsToBecreated - i].family.add(
                                self._agentsToAdd[agentsToBecreated - y].id)
                            ...
                            # INFECTION
                            infected = np.random.choice(["S", "E", "A", "I"], p=[0.970, 0.015, 0,
                                                                                 0.015])  # p=[0.985, 0, 0.015]) p=[0.970, 0.015,0 ,0.015])#
                            if infected == "I":
                                self._agentsToAdd[agentsToBecreated - i].machine = SEAIHRD_covid(agentsToBecreated - i,
                                                                                                 "I",
                                                                                                 age_())
                                ...
                                self._agentsToAdd[agentsToBecreated - i].R0_contacts[
                                    self.DateTime.strftime('%Y-%m-%d')] = [0,
                                                                           round(
                                                                               1 /
                                                                               self._agentsToAdd[
                                ...
                                self._agentsToAdd[agentsToBecreated - i].R0_contacts[
                                    ...
                                    [0, round(1 / self._agentsToAdd[agentsToBecreated - i].machine.rate['rEI']) + round(
                                        1 / self._agentsToAdd[agentsToBecreated - i].machine.rate['rIR']) -
                                     self._agentsToAdd[
                            ...
                            if np.random.choice([True, False],
                                                p=[self.employment_rate, 1 - self.employment_rate]) and 5 < \
                                    self._agentsToAdd[agentsToBecreated - i].machine.age < 65:
                                workplaces = random.sample(
                                    list(range(len(self._agentsToAdd) - Workplaces, len(self._agentsToAdd))),
                                    Workplaces)
                                for workplace in workplaces:
                                    if self._agentsToAdd[workplace].total_capacity > len(
                                            self._agentsToAdd[workplace].get_workers()):
                                        self._agentsToAdd[agentsToBecreated - i].workplace = self._agentsToAdd[
                                            workplace].id
                                        self._agentsToAdd[workplace].add_worker(
                                            self._agentsToAdd[agentsToBecreated - i].id)
                                    break
                ...
                # node = ox.nearest_nodes(self.Ped_Map.G, agent_loc.x, agent_loc.y, return_dist=False)
                # node = ox.nearest_nodes(self.Ped_Map.G, locations[i].x, locations[i].y, return_dist=False)
                position = (self.Ped_Map.G.nodes[node]['x'], self.Ped_Map.G.nodes[node]['y'])
    ...
    def population_density(self):
        ...
        def activate_alarm_state(self):
            ...
            if 'remote-working' in self.alarm_state.keys() and self.alarm_state[
                'remote-working'] < self.employment_rate:
