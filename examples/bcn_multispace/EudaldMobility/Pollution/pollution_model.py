...
class Interpolation_Diffusion_Model():
    def __init__(self, boundaries, init_time):
        ...
        self.corner_0_0 = (self.boundaries["w"],self.boundaries["s"])
        self.corner_1_1 = (self.boundaries["e"], self.boundaries["n"])
        ...
        for index, item in self.dataset.loc[self.dataset['Dia'] == 1].iterrows():
            stations[item['Estacio']] = (item['Latitud'], item['Longitud'])
    ...
    def mask(self):
        # Create mask 
        city = ox.geocode_to_gdf('Barcelona, Spain')
        city = ox.project_gdf(city)
        m = [[False for i in range(len(self.X))] for j in range(len(self.Y))]
        for i in range(len(self.X)):
            for j in range(len(self.Y)):
                m[i][j] = city.geometry.contains(Point(self.X[i][j],self.Y[i][j])).item()
        mi = ~np.matrix(m)
        return mi

    def plot_2D(self, mask = None):
        # plot
        try:
            z_plot_values = self.z_values.copy()
            z_plot_values[mask] = np.nan
        except:
            z_plot_values = self.z_values.copy()

        fig = plt.figure()
        ax = fig.add_subplot(111)
        plt.contourf(self.X,self.Y,z_plot_values,np.arange(min(map(min, self.z_values))-10, max(map(max, self.z_values))+10, 1), cmap=plt.cm.autumn)
