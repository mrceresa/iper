...
class BasicHuman(Agent):
    ...
    def move(self):
        ...
        if self.meet_w_friend is None:
            ...
            new_position = min(possible_steps, key=lambda c: euclidean(c, self.meet_w_friend.pos))  # check shortest step towards friend new position
...
class BCNCovid2020(Model):
    ...
    def __init__(self, N, basemap, width=50, height=50):
        ...
        self.datacollector = DataCollector(
            {"INF": get_infected_count, "SUSC": get_susceptible_count, "REC": get_recovered_count,
             "DEAD": get_dead_count, },
    ...
    def update_DC_table(self):
        ...
        next_row = {'Steps': step_num, 'Infected': get_infected_count(self),
                    'Susceptible': get_susceptible_count(self), 'Recovered': get_recovered_count(self),
                    'Dead': get_dead_count(self)}
