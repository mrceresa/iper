...
def setDatabase(args):
    df = pd.read_csv("data/2020_seguiment-covid19-bcn.csv", delimiter=',')
    df['Data_Indicador'] = pd.to_datetime(df['Data_Indicador'], format='%Y-%m-%d')
    df = df[df.Territori == 'Barcelona']

    df_casos_diaris = df[df.Nom_Indicador == 'Casos de Covid19 a Barcelona']
    df_casos_diaris.drop(df_casos_diaris.columns[[1, 2, 3, 4, 6, 7]], axis=1, inplace=True)
    df_casos_diaris.rename(columns={'Data_Indicador': 'date', 'Valor': 'confirmed'}, inplace=True)

    print(df_casos_diaris.head())
    df_hospitals = df[df.Nom_Indicador == 'Casos de Covid19 als hospitals de Barcelona']

    df_hospitals_altes = df_hospitals[df_hospitals.Nom_Variable == 'Altes']
    df_hospitals_altes.drop(df.columns[[1, 2, 3, 4, 6, 7]], axis=1, inplace=True)
    df_hospitals_altes.rename(columns={'Data_Indicador': 'date', 'Valor': 'recovered'}, inplace=True)

    df_hospitals_ingressats = df_hospitals[df_hospitals.Nom_Variable == 'Ingressats']
    df_hospitals_ingressats.drop(df.columns[[1, 2, 3, 4, 6, 7]], axis=1, inplace=True)
    df_hospitals_ingressats.rename(columns={'Data_Indicador': 'date', 'Valor': 'hosp'}, inplace=True)

    df_morts_totals = df[df.Nom_Indicador == 'Defuncions a Barcelona (estimació)']
    df_morts_observades = df_morts_totals[df_morts_totals.Nom_Variable == 'Observades']
    df_morts_observades.drop(df.columns[[1, 2, 3, 4, 6, 7]], axis=1, inplace=True)
    df_morts_observades.rename(columns={'Data_Indicador': 'date', 'Valor': 'deaths'}, inplace=True)

    merged_inner = pd.merge(left=df_casos_diaris, right=df_hospitals_altes, left_on='date', right_on='date')
    merged_inner2 = pd.merge(left=merged_inner, right=df_hospitals_ingressats, left_on='date', right_on='date')
    merged_inner3 = pd.merge(left=merged_inner2, right=df_morts_observades, left_on='date', right_on='date')

    merged_inner3['icu'] = 0
    merged_inner3['tests'] = 0
    merged_inner3['vent'] = 0

    doFit(args, merged_inner3)
