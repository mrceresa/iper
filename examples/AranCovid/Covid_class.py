...
class VirusCovid(object):
    ...
    def __init__(self, d):
        ...
        # from EXP to INF
        self.incubation_days = d["incubation_days"]
        self.incubation_days_sd = int(d["incubation_days"] * 0.4)

        # from INF to REC - HOSP
        self.infection_days = d["infection_days"]
        self.infection_days_sd = int(d["infection_days"] * 0.4)

        # from REC to SUSC
        self.immune_days = d["immune_days"]
        self.immune_days_sd = int(d["immune_days"] * 0.4)

        # from HOSP to REC - DEATH
        self.severe_days = d["severe_days"]
        self.severe_days_sd = int(d["severe_days"] * 0.4)

        self.ptrans = d["pTrans"]
        self.pSympt = d["pSympt"]  # probability to present the symptoms
        self.pTest = d["pTest"] # probability of test of true positive
        self.death_rate = d["death_rate"]
        self.severe_rate = d["severe_rate"]
